...
int main(void){
    double time=0.4+10*PI;          // どこまで計算するか
    double a=-0.5;             // 始点
    double b=0.5;              // 終点
    ...
    for(int step=0; step<N; step++){
        ...
        for(int i=0; i<I; i++){
            ...
            double uL = u[i]+(1-k)*(u[i]-u[im])/4.+(1+k)*(u[ip]-u[i])/4.;
            double uR = u[ip]-(1+k)*(u[ip]-u[i])/4.-(1-k)*(u[ipp]-u[ip])/4.;
    ...
    // ファイルの用意
    char filename[256];
    sprintf(filename, "out_2nd_%02d.data", (int)((time+0.003)*100));
