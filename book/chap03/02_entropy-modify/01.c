...
int main(void){
    ...
    for(int step=0; step<N; step++){
        ...
        for(int i=0; i<I-1; i++){
            ...
            /*エントロピー補正のため追記*/
            double cs0=(gamma-1)*(H0-pow(v0, 2.)/2.);
            double cs1=(gamma-1)*(H1-pow(v1, 2.)/2.);

            double lambda[3]={bv-bcs, bv, bv+bcs};
            
            /*エントロピー補正のため追記*/
            double lambda0[3]={v0-cs0, v0, v0+cs0};
            double lambda1[3]={v1-cs1, v1, v1+cs1};

            /*エントロピー補正*/
            double eps[3];
            for(int j=0; j<3; j++){
                eps[j]=fmax(0, fmax(lambda[j]-lambda0[j], lambda1[j]-lambda[j]));
                if(fabs(lambda[j])<eps[j]){
                    lambda[j]=(pow(lambda[j], 2.)/eps[j]+eps[j])/2.;
                }
            }
