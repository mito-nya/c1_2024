...
int main(void){
    ...
    for(int step=0; step<N; step++){
        ...
        for(int i=0; i<I; i++){
            ...
            // 制限関数の導入
            double uL = u2[i]+0.5*minmod(u2[ip]-u2[i], u2[i]-u2[im]);
            double uR = u2[ip]-0.5*minmod(u2[ip]-u2[i], u2[ipp]-u2[ip]);
        ...
        for(int i=0; i<I; i++){
            int im = (i+I-1)%I;
            tmp[i] = 0.5*(u[i]+u2[i]-dt/dx*(flux[i]-flux[im]));
        }
    ...
    // ファイルの用意
    char filename[256];
    sprintf(filename, "out_heun_%02d.data", (int)((time+0.003)*100));
