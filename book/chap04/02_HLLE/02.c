...
int convert(double *UL, double *UR, double *FL, double *FR, double *flux){
    ...
    if (rhoL <= 0 || rhoR <= 0) {
        printf("Error: Negative or zero density encountered.\n");
        exit(1);
    }
    ...
    double ptL = pL+BL/2.;
    double csL = sqrt(fmax(0, gamma*pL/rhoL));
    ...
    double ptR = pR+BR/2.;
    double csR = sqrt(fmax(0, gamma*pR/rhoR));
    ...
    *(FR+7) = (eR+ptR)*uR-bxR*(bxR*uR+byR*vR+bzR*wR);
    ...
    if (SL == SR) {
        printf("Error: SL and SR are equal, leading to division by zero.\n");
        exit(1);
    }
